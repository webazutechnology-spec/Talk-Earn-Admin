...
@section('content')
    ...
    <div class="card border-top border-0 border-4 border-primary">
        <div class="card-body">
            ...
            <hr>
            <form action="{{ route('state-store') }}" method="POST" class="row g-3" enctype="multipart/form-data">
                @csrf

                <div class="col-md-3">
                    <label for="country" class="form-label">Country</label>
                    <select name="country" class="single-select @error('country') is-invalid @enderror" id="country">
                        <option selected disabled value="">Choose...</option>
                        @foreach ($countrie as $country)
                        <option value="{{ $country->id }}" @selected($country->id == old('country'))>{{ $country->name }}</option>
                        @endforeach
                    </select>
                    @error('country')
