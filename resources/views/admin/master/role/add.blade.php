...
@section('content')
    ...
    <div class="card border-top border-0 border-4 border-primary">
        <div class="card-body">
            ...
            <hr>
            <form action="{{ route('role-store') }}" method="POST" class="row g-3" enctype="multipart/form-data">
            @csrf
                <div class="col-md-3">
                    ...
                    <select name="type" class="form-select @error('type') is-invalid @enderror" id="type" required>
                        ...
                        <option value="Employee" @selected('Employee' == old('type'))>Employee</option>
                        <option value="User" @selected('User' == old('type'))>User</option>
                    </select>
                    @error('type')
                ...
                <div class="col-md-3">
                    ...
                    <input type="number" name="order_by" class="form-control @error('order_by') is-invalid @enderror" placeholder="Order By Name" id="order_by" value="{{ old('order_by') }}">
                    @error('order_by')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-3">
                    ...
                    <select name="show" class="form-select @error('show') is-invalid @enderror" id="show" required>
                        <option selected disabled value="">Choose...</option>
                        <option value="Yes" @selected('Yes' == old('show'))>Yes</option>
                        <option value="No" @selected('No' == old('show'))>No</option>
                    </select>
                    @error('show')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
